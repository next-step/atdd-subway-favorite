...
public class FavoriteMemberClient implements MemberClient {
    ...
    @Override
    public ApplicationTokenResponse getMemberToken(ApplicationTokenRequest applicationTokenRequest) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Accept", MediaType.APPLICATION_JSON_VALUE);

        HttpEntity<MultiValueMap<String, String>> httpEntity = new HttpEntity(
                applicationTokenRequest, headers);
        RestTemplate restTemplate = new RestTemplate();

        return restTemplate
                .exchange(tokenUrl, HttpMethod.POST, httpEntity, ApplicationTokenResponse.class)
                .getBody();
    }
}
