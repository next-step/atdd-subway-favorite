...
public class FavoriteAcceptanceTest extends AcceptanceTest {
  ...
  void 즐겨찾기_생성_실패_인증_토큰_누락() {
    // when
    final var response = 즐겨찾기_생성_요청(강남역.getId(), 양재역.getId(), null);
  ...
  /**
   * When 출발역과 도착역이 같은 구간에 대한 즐겨찾기 생성을 요청하면
   * Then 에러가 발생한다.
   */
  ...
  /**
   * When 연결되지 않는 구간에 대한 즐겨찾기 생성을 요청하면
   * Then 에러가 발생한다.
   */
  @DisplayName("즐겨찾기 생성 실패 - 연결되지 않는 구간")
  @Test
  void 즐겨찾기_생성_실패_연결되지_않는_구간() {
    // when
    final var response = 즐겨찾기_생성_요청(강남역.getId(), 강남역.getId(), accessToken);
