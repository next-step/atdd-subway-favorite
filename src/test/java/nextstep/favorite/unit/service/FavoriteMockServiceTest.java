...
import static nextstep.common.Constant.홍길동_비밀번호;
import static nextstep.common.Constant.홍길동_이메일;
import static nextstep.common.Constant.홍길동_나이;
...
public class FavoriteMockServiceTest {
    ...
    private final Station 을지로입구역 = Station.from(Constant.을지로입구역);
    private final Long 을지로입구역_ID = 4L;
    private Line 신분당선;
    ...
    void 경로가_없는_구간을_즐겨찾기에_추가() {
        ...
        when(stationService.findById(을지로입구역_ID)).thenReturn(을지로입구역);

        // when & then
        assertThatThrownBy(() -> favoriteService.createFavorite(LoginMember.from(홍길동_이메일), AddFavoriteRequest.of(강남구청역_ID, 을지로입구역_ID)))
