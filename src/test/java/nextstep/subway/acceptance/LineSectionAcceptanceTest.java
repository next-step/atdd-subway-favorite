...
class LineSectionAcceptanceTest extends AcceptanceTest {
    ...
    @Test
    void addLineSection_admin() {
        // when
        Long 정자역 = 지하철역_생성_요청(관리자, "정자역").jsonPath().getLong("id");
        ExtractableResponse<Response> 구간생성_결과 = 지하철_노선에_지하철_구간_생성_요청(관리자, 신분당선, createSectionCreateParams(양재역, 정자역));

        // then
        assertThat(구간생성_결과.statusCode()).isEqualTo(HttpStatus.OK.value());
    ...
    /**
     * When 관리자가 아닌 자가 지하철 노선에 새로운 구간 추가를 요청 하면
     * Then 노선에 새로운 구간이 추가되지 않는다.
     */
    @DisplayName("지하철 노선에 구간을 등록")
    @Test
    void addLineSection_member() {
        // when
        Long 정자역 = 지하철역_생성_요청(관리자, "정자역").jsonPath().getLong("id");
        ExtractableResponse<Response> 구간생성_결과 = 지하철_노선에_지하철_구간_생성_요청(사용자, 신분당선, createSectionCreateParams(양재역, 정자역));

        // then
        assertThat(구간생성_결과.statusCode()).isEqualTo(HttpStatus.UNAUTHORIZED.value());
        ExtractableResponse<Response> response = 지하철_노선_조회_요청(신분당선);
        assertThat(response.statusCode()).isEqualTo(HttpStatus.OK.value());
        assertThat(response.jsonPath().getList("stations.id", Long.class)).doesNotContain(정자역);
    }
