...
class PathRepositoryTest {
    ...
    void setUp() {
        ...
        pathRepository = new PathRepository(lineRepository, shortestPathFinder, stationMapper);
    ...
    @Test
    void test() {
        ShortestPathResponse shortestPathResponse = pathRepository.get(교대역.getId(), 양재역.getId()).get();
        assertAll(
    ...
    void test2() {
        //같은 경로를 두번 호출 할 경우
        pathRepository.get(교대역.getId(), 양재역.getId());
        ShortestPathResponse shortestPathResponse = pathRepository.get(교대역.getId(), 양재역.getId()).get();
