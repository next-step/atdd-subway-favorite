...
public enum GithubResponses {
  ...
  public static GithubResponses findByCode(String code) {
    return Arrays.stream(GithubResponses.values()).filter(response -> response.getCode().equals(code))
        .findAny().orElseThrow(() -> new NoSuchElementException());
  }
  public static GithubResponses findByAccessToken(String accessToken) {
    return Arrays.stream(GithubResponses.values()).filter(response -> response.getAccessToken().equals(accessToken))
        .findAny().orElseThrow(() -> new NoSuchElementException());
  }
